...
class AppController extends Controller
{
	...
	public function getDataHome(Request $request)
	{
		try {
			return array(
                'status' => 'success',
				'episodes' => $this->episode->app_episodiosReciente(),
				'animeN' => $this->anime->app_animesRecientes(),
				'animeP' => $this->anime->app_animesPopulares(),
				'animeV' => $this->anime->app_animesMasVistos()
			);
		} catch (Exception $e) {
			return array(
                'status' => 'error',
	...
	{
		try {
			$anime = $this->anime->app_obtenerAnime($request->slug);
			if(!$anime)
				throw new Exception("Anime no encontrado", 1);
			$episodes = $this->episode->app_episodiosAnime($anime->id);
			$recomendations = $this->anime->app_recomendacionesAnime($anime);
			return array(
                'status' => 'success',
				'anime' => $anime,
				'episodes' => $episodes,
	...
	{
		try {
			$animes = $this->anime->app_animesListaFiltros($request);
			return array(
                'status' => 'success',
				'animes' => $animes
			);
